...
どの瞬間を選ぶか(最重要):
- 選ぶのは **そのセミナーの核心的な内容**（アーキテクチャ図、データ・グラフ、
  デモ画面、比較表、結論・まとめのスライドなど）に限ること。
- 次のような**内容が薄い・構造的なだけのスライドは選ばないこと**：
  アジェンダ／目次、タイトルスライド、登壇者紹介、章の区切りだけのスライド、
  「ご清聴ありがとうございました」等の締めのスライド。
- 複数の候補がある場合は、そのセクションの論旨が最もよく伝わる 1 箇所を選ぶこと。
  「セクションの真ん中あたり」のような機械的な選び方はしないこと。
...
- このセクションには **必ず 1 個以上、最大 {max_captures} 個**のマーカーを入れること。
  アジェンダ紹介のような内容が薄いセクションであっても、その中で最も具体的な
  話題（次に来る本題への言及など）を 1 つ選ぶこと。0 個にはしないこと。
"""
