...
- 複数の候補がある場合は、そのセクションの論旨が最もよく伝わる箇所を選ぶこと。
  「セクションの真ん中あたり」のような機械的な選び方はしないこと。
- 個数の目安は {max_captures} 個だが、これは上限ではない。このセクションに
  重要な瞬間が複数あれば、章の区切りに関係なく全てマーカーを入れてよい。
  画像点数が多くなること自体は問題ではないので、遠慮しないこと。
- 逆に、内容が薄いセクションで無理に複数入れる必要はない。水増しのための
  マーカーは入れないこと。
...
- このセクションには **必ず 1 個以上**のマーカーを入れること。
