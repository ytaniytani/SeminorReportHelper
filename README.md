...
**切り出し範囲を画像で指定する**：「画像で範囲を指定」を開くと、スクリーンショット
画像をドラッグ&ドロップ/貼り付けするほか、「動画からフレームを取得」でいま選択中の
動画そのものをその場で読み込み、シークバーで好きな瞬間まで移動してそのフレームを
使うこともできる（サーバーへのアップロードは発生しない）。取得した画像上をドラッグ
すると、その矩形が「画像の切り出し範囲」欄に反映される。複数動画を選択している
場合は先頭の動画が対象になる。
