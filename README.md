...
進捗画面には、詳細度・LLMプロバイダ/モデル・Whisperモデル・画像設定・切り出し範囲の
有無など、今どの条件で実行しているかも表示される（プロバイダ/モデルを未指定にした
場合は `.env` の既定値が使われるため、特に「今どのモデルで動いているか」が分かる）。
...
**切り出し範囲を動画で指定する**：「動画で範囲を指定」を開くと、いま選択中の動画が
ブラウザ内でその場で読み込まれる（サーバーへのアップロードは発生しない）。シーク
バーで切り出したい画面が映っている瞬間まで移動し、「この位置のフレームを使う」を
押すとそのフレームが表示される。表示された画像上をドラッグすると、その矩形が
「画像の切り出し範囲」欄に反映される。複数動画を選択している場合は先頭の動画が対象。

**処理を途中で止める**：進捗画面の「処理を中止」ボタンで止められる。押すと確認
ダイアログが出て、OK を選ぶと中止される。実行中のスレッドは強制終了できないため、
即座ではなく**現在の工程の区切り（文字起こしのセグメント単位、LLM 呼び出し 1 回分、
画像 1 枚分）で停止する**。文字起こし中なら数秒、LLM の実行中は応答待ちの分だけ
（長くても 1 分程度）かかる。アップロード中に押した場合は転送そのものを打ち切る。

中止した動画のレポートは作成されないが、**文字起こしが終わっていればキャッシュに
残る**ため、同じ動画をやり直す場合は次回が速い。複数動画の処理中に中止すると、
処理中の動画と未処理の動画が中止され、完了済みのレポートはそのまま残る。

**複数動画をまとめて処理する**：mp4 を複数選択（またはドラッグ&ドロップ）すると、
同じ詳細度・LLMプロバイダ/モデル・切り出し範囲などの条件のまま、1本ずつ順番に
処理される（GPU を使う文字起こしが重いため、同時実行はしない）。進捗画面に
「動画 2/3: seminar2.mp4」のように現在処理中の動画と全体の状況が表示され、
完了した動画から順に一覧に「report.html を開く」リンクが出る。全件終わると
成功/失敗の一覧画面に切り替わる。

複数選択時は「完了時にレポートを新しいタブで開く」は無効になる。ブラウザは
クリックなどの操作に対して同期的に開いたポップアップしか通さないため、
非同期に完了していく2本目以降の動画では自動オープンがブロックされてしまう。
代わりに、完了した動画ごとのリンクから開く。
...
# 複数動画を同じ条件でまとめて処理（1本ずつ順番に実行し、失敗しても次に進む）
uv run seminar-report run seminar1.mp4 seminar2.mp4 seminar3.mp4 -d detailed
...
| `--max-captures` | 画像枚数の目安（章あたりの上限。全体の厳密な総量制限ではない） |
...
| プリセット | 目標文字数 | セクション数 | 画像枚数の目安 |
|---|---|---|---|
| `brief` 簡潔 | 800 | 3〜4 | 3 |
| `standard` 標準 | 2,500 | 5〜7 | 6 |
| `detailed` 詳細 | 6,000 | 8〜12 | 12 |

`--chars` を指定した場合は、その文字数からセクション数と画像枚数の目安を自動算出する。

画像枚数はあくまで目安であり、厳密な上限ではない。重要な瞬間が特定の章に
集中している場合（「レポートへの要望」で内容を絞り込んだ場合など）は、
目安を超えて画像が入ることがある。逆に内容が薄い章では無理に増やされない。
章をまたいだ全体の枚数調整は行わず、各章が「重要かどうか」だけで判断する。
...
複数動画を指定した場合は動画ごとに `<動画名>/` のサブディレクトリへ分かれる。
`--out` を指定すると、その配下に動画ごとのサブディレクトリが作られる
（1本だけを処理する場合は `--out` をそのまま出力先として使う）。
...
| 中止ボタンを押してもすぐ止まらない | 仕様。工程の区切りまで待ってから停止する（LLM の応答待ち中は最大 1 分ほど）。「停止しています…」の表示中は受け付け済み |
